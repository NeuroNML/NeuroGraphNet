...
class MessagePassing(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        # Compute messages from source nodes: [num_edges, out_channels]
        ...
        # message[i]: source[i] -> target[i]
        # For node v: target[i] == v  -> finds all messages sent to v and aggregates them
        if self.aggr == "add":
            aggregated = scatter_add(
                message, target, dim=0, dim_size=x.size(0)
            )  # Take all targets
        elif self.aggr == "mean":
        ...
        )  # Concatenate previous node features with new and apply NN
